                               ...
                               uint8_t *packet, uint16_t size)
{
    ...
    case HCI_EVENT_DISCONNECTION_COMPLETE: {
        // Classic bonding deliberately disconnects its ACL. It is not our Mouse.
        if (hci_event_disconnection_complete_get_status(packet) != ERROR_CODE_SUCCESS ||
            g_connection_handle == HCI_CON_HANDLE_INVALID ||
            hci_event_disconnection_complete_get_connection_handle(packet) != g_connection_handle)
            break;
        const bool was_ready = g_state == BLE_HOGP_STATE_READY;
...
{
    return blu2usb_bt_runtime_start(blu2usb_ble_hogp_session_setup);
}
