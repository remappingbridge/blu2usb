...
void sleep_ms(uint32_t ms) {(void)ms;assert(!"Core1 must not substitute the LCD boot gate");}
int cyw43_arch_init(void) {assert(phase==1 && flash_inits==2);phase=3;return 0;}
...
static void prepare(void) {assert(phase==8);phase=80;}
static void classic_setup(void) {assert(phase==80);phase=9;}
...
int main(void) {
    blu2usb_bt_boot_gate_t gate = {0};
    assert(!blu2usb_bt_boot_due(&gate,false,1000));
    assert(!blu2usb_bt_boot_due(&gate,true,2000));
    assert(!blu2usb_bt_boot_due(&gate,true,2499));
    assert(blu2usb_bt_boot_due(&gate,true,2500));
    assert(!blu2usb_bt_boot_due(&gate,true,9000));
    gate=(blu2usb_bt_boot_gate_t){0};
    assert(!blu2usb_bt_boot_due(&gate,true,UINT32_MAX-100));
    assert(!blu2usb_bt_boot_due(&gate,true,398));
    assert(blu2usb_bt_boot_due(&gate,true,399));
    gate=(blu2usb_bt_boot_gate_t){0};
    assert(!blu2usb_bt_boot_due(&gate,true,0));
    assert(!blu2usb_bt_boot_due(&gate,false,400));
    assert(!blu2usb_bt_boot_due(&gate,true,500));
    assert(blu2usb_bt_boot_due(&gate,true,1000));
    assert(!blu2usb_bt_runtime_start(NULL,classic_setup,prepare));
    assert(blu2usb_bt_runtime_start(ble_setup,classic_setup,prepare));
    assert(!blu2usb_bt_runtime_start(ble_setup,classic_setup,prepare));
